...
class ReportManager {
  ...
  createInfoBox(item, isSurvey = false) {
    ...
    magVal.classList.add(!item.id.split('-')[0].includes('000') ? 'isNum' : null);
  ...
  createReportItem(item, isSurvey = false) {
    ...
    wrapper.setAttribute('trem-url', item.trem ? `https://api-1.exptech.dev/file/trem_info.html?id=${item.trem}` : '');
