...
class EewManager {
  ...
  bindEvents() {
    TREM.variable.events.on('EewRelease', (ans) => {
      this.eew_cache[ans.data.id] = ans.data;
      this.show_eew(false);
    ...
    TREM.variable.events.on('EewEnd', (ans) => {
      this.removeEewLayersAndSources(ans.data.id);
      Reflect.deleteProperty(this.eew_cache, ans.data.id);
      this.show_eew(true);
    });
    ...
    setInterval(this.show_eew, 5000);
  }

  show_eew(rotation = true) {
    let count = 0;
    const eew_list = Object.keys(this.eew_cache);
    ...
    if (count && eew_list.length) {
      ...
      if (this.eew_cache[eew_list[this.eew_rotation]]) {
        if (!TREM.constant.SHOW_TREM_EEW && this.eew_cache[eew_list[this.eew_rotation]].author == 'trem') {
          this.eew_rotation++;
          if (this.eew_rotation >= eew_list.length) {
            this.eew_rotation = 0;
          }
        }
        else {
          const eew = this.eew_cache[eew_list[this.eew_rotation]];
          ...
          const unitText = `${eew.author.toUpperCase()}${count == 1 ? '' : ` ${this.eew_rotation + 1}/${count}`}`;
  ...
  createCircleFeature(center, radius, steps = 256) {
    const coordinates = [[]];
    const km = radius;
  ...
  removeEewLayersAndSources(eewId) {
    const layerIds = [
