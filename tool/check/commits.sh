#!/usr/bin/env bash
# Rejects a commit message the release notes cannot be built from.
#
# Ported from DPIP (tool/check/commits.sh). This is a gate rather than a linter
# because the message *is* the changelog: the release notes are read out of
# these bodies and published, so a commit that does not carry both languages
# leaves a hole in a note that users read. There is no second place to fix it —
# the message is immutable once pushed, and the only repair is a rebase.
# Catching it at the gate is what keeps that repair small.
#
...
readonly KNOWN_LOCALES='zh-Hant zh-Hans zh-Hant-HK en-US ja-JP ko-KR'
...
check_one() { # <subject> <body> <label>
  local subject body short bad entries platform
  subject="$1"
  body="$2"
  short="$3"
  bad=0
  ...
  # limit fails a commit that passed this very gate while the PR was open.
  ...
  *.)
  ...
  # Testing for ASCII rather than for CJK ranges: one portable expression
  # instead of a `grep -P` that BSD grep does not have, and it catches kana,
  # Hangul and emoji in the same breath.
  ...
  #    accountable for the change. Judged by *who* the trailer credits: a human
  #    Co-authored-by is how attribution survives a squash, so banning the
  #    trailer outright would destroy what it was meant to protect.
  if printf '%s' "$body" |
  ...
  # 3. Changelog lines. These are what the release notes publish, so a mistake
  #    here is a hole in something users read.
...
EOF
  ...
  if printf '%s' "$subject" | grep -Eq "^($USER_FACING)(\(|:)"; then
    if [ -z "$entries" ]; then
      ...
      # one language and forgotten in another silently ships a shorter list.
...
if [ "$mode" = message ]; then
  check_one "$(head -n 1 "$msg_file")" "$(tail -n +2 "$msg_file")" "draft"
else
