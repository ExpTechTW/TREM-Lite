...
if [ "${1:-}" = "--write" ]; then
  ...
  # it recomputes: its checkout is shallow, so it cannot count this week's
  # snapshots, and the label would come out wrong. The release workflow
  # therefore passes it down.
  ...
  # tauri.conf.json is edited as JSON rather than by pattern: a first-match
  ...
  CONF="$root/apps/desktop/src-tauri/tauri.conf.json" SEMVER="$semver" bun -e '
